 ...
 */
export async function getUnreadNotificationCountController(req: AuthenticatedRequest, res: Response) {
  try {
    if (!req.user?.id) {
      return res.status(401).json({
        status: "error",
    ...
    const count = await notificationRepository.getUnreadNotificationCount(req.user.id);
...
export async function markNotificationAsReadController(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const notification = await notificationRepository.markNotificationAsRead(notificationId, req.user.id);
...
export async function deleteNotificationController(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    await notificationRepository.deleteNotification(notificationId, req.user.id);
