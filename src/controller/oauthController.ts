...
export function googleCallbackController(
  ...
  passport.authenticate("google", { session: false }, (err, data) => {
    if (err) {
      ...
      return res.redirect(`${frontendUrl}/auth/oauth-error?provider=google`);
    }
    ...
    return res.redirect(
      `${frontendUrl}/auth/oauth-success?token=${data.token}`
    );
  })(req, res, next);
}
 ...
 */
export function githubCallbackController(
  req: Request,
  res: Response,
  next: NextFunction
) {
  passport.authenticate("github", { session: false }, (err, data) => {
    if (err) {
      ...
      return res.redirect(`${frontendUrl}/auth/oauth-error?provider=github`);
    }
