...
export async function uploadFile(input: FileUploadInput) {
  ...
  const file = await db
    ...
    .values({
      ...
      file_size: BigInt(input.size), // Use file_size instead of size
      file_path: `${input.workspaceId}/${fileName}`, // Use file_path instead of path
...
export async function getFilePath(id: string, userId: string) {
  ...
  if (file.is_public) {
    return {
      path: path.join(UPLOAD_DIR, file.file_path),
